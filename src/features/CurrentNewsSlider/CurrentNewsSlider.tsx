  ...
  const CurrentNewsSlider: React.FC<CurrentNewsReelProps> = ({ updateIntervalMinutes = 15 }) => {
    ...
    return (
      <section className="CurrentNewsSlider">
        <CurrentNewsCards updateIntervalMinutes={updateIntervalMinutes} ref={currentCardsSliderRef} />
        <div className="CurrentNewsSlider__wrapperBtn">
          <button className="CurrentNewsSlider__btn" onClick={() => handleScroll("left")}>
          <img src={btnLeft} alt="left" width="24" height="24" />
          </button>
          <button className="CurrentNewsSlider__btn" onClick={() => handleScroll("right")}>
          <img src={btnRight} alt="right" width="24" height="24" />
          </button>
        </div>
      </section>
    );
